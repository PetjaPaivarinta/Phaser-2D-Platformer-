    ...
    create ()
{
    ...
    // add title text
    this.add.text(this.sys.game.config.width / 2, this.sys.game.config.height / 2 - 300, 'Main Menu', { fontFamily: 'Comic Sans', fontSize: '120px', color: '#ffffff', fontWeight: 'bold'}).setOrigin(0.5).setDepth(1);
    // make a black box bigger than the screen
    this.add.rectangle(this.sys.game.config.width / 2, this.sys.game.config.height / 2, this.sys.game.config.width, this.sys.game.config.height, 0).setOrigin(0.5).setDepth(0);
    // add box around text
    this.box = this.add.rectangle(this.sys.game.config.width / 2, this.sys.game.config.height / 2, 300, 100, '595959').setOrigin(0.5);
    // set depth of text to be above 
    let text = this.add.text(this.sys.game.config.width / 2, this.sys.game.config.height / 2, 'Start', { fontFamily: 'Comic Sans', fontSize: '100px', color: '#ffffff', fontWeight: 'bold'}).setOrigin(0.5).setDepth(1);
}
